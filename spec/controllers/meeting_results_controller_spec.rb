...
RSpec.describe MeetingResultsController, type: :controller do
  ...
  describe 'POST #update_passages' do
    context "for a logged-in user, with a valid request (user is a team manager and the meeting exists)," do
      ...
      # XXX The following has been extensively tested by hand and so this spec has very low priority:
      # TODO It updates the meeting#updated_at only if some passages has been actually updated
      xit "updates the meeting" do
        expect{
          # Needs:
          # - a valid "pas_" + passage.id param with a new, different string timing each time the spec is called
          post( :update_passages, params: { id: 15202 } )
        }.to change{ Meeting.find(15202).updated_at }
      end
    end
    ...
    context "for a logged-in team manager, but w/o editable results on the meeting," do
      ...
      it "accepts the request (redirecting to #edit_relay_swimmers)" do
        post :update_passages, params: { id: uneditable_meeting_id_for_team_1 }
        expect( response.status ).to redirect_to(
          meeting_results_edit_passages_path(uneditable_meeting_id_for_team_1, locale: I18n.default_locale)
        )
      end
      it "does not update the meeting" do
        expect{
          post( :update_passages, params: { id: uneditable_meeting_id_for_team_1 } )
        }.not_to change{ Meeting.find(uneditable_meeting_id_for_team_1).updated_at }
      end
    end
  end
  #-- -------------------------------------------------------------------------
  ...
  describe "GET #update_relay_swimmers" do
    ...
    context "for a logged-in team manager, but w/o editable results on the meeting," do
      ...
      it "accepts the request (redirecting to #edit_relay_swimmers)" do
        post :update_relay_swimmers, params: { id: uneditable_meeting_id_for_team_1 }
        expect( response.status ).to redirect_to(
          meeting_results_edit_relay_swimmers_path(uneditable_meeting_id_for_team_1, locale: I18n.default_locale)
        )
      end
      it "does not update the meeting" do
        expect{
