...
# TODO Next: (1 token extractor x example/context)
#        FinResultConsts::TOK_EXT_MEETING_HEADER_ORGANIZATION
...
# FIXME Adjust regexp:
# expected "4/15 Gennaio 2012" to match "14/15 Gennaio 2012"
...
# FIXME Adjust regexp:
# expected "scita Team Romagna asd" to match "Rinascita Team Romagna asd"
    xit "splits correctly in tokens only the recognized feed lines" do
      feed.each_with_index do |feed_line, index|
        token = subject.tokenize( feed_line )
        expect( token ).to match( expected_tokens[index] )
