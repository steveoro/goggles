...
describe SwimmersHelper do
  ...
  describe "#ask_confirm_link" do
    ...
    it "renders the link to request a friendship for an associated swimmer" do
      @user.swimmer = @swimmer
      @swimmer.associated_user = @user
      expect( helper.ask_confirm_link( @swimmer ) ).to include( I18n.t('social.menu_social_confirm') )
      expect( helper.ask_confirm_link( @swimmer ) ).to include( social_association_confirm_path(id: @swimmer.id) )
    end
  end
end
