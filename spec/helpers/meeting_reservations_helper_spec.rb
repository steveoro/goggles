...
RSpec.describe MeetingReservationsHelper, type: :helper do
  ...
  describe "#link_to_reservation_csv_data_export()" do

    context "when given a nil meeting and current user," do
      subject { helper.link_to_reservation_csv_data_export( nil, nil ) }
    ...
    context "when given a nil meeting," do
      subject { helper.link_to_reservation_csv_data_export( build(:user), nil ) }
    ...
    context "when given a nil current_user," do
      subject { helper.link_to_reservation_csv_data_export( nil, old_meeting ) }
    ...
    context "when given valid parameters," do
      shared_examples_for "correctly rendering the event CSV export link button" do
        it "is not nil" do
          expect( subject ).not_to be_nil
        end
        it "returns a non-empty text" do
        ...
        it "contains the image name" do
          expect( subject ).to include('page_white_excel')
        end
      end


      context "[params: meeting closed W/O reservations and a valid user]," do
        subject { helper.link_to_reservation_csv_data_export( user_for_reservation, old_meeting ) }
        it "returns nil" do
          expect( subject ).to be nil
        end
      end
      ...
      context "[params: new meeting (open) W/O reservations and a valid user]," do
        subject { helper.link_to_reservation_csv_data_export( team_manager.user, new_meeting ) }
        it "returns nil" do
          expect( subject ).to be nil
        end
      end
      ...
      context "[params: meeting WITH an existing reservation and a valid user (for the reservation)]," do
        ...
        subject { helper.link_to_reservation_csv_data_export( user_for_reservation, meeting_with_reservation ) }

        it_behaves_like "correctly rendering the event CSV export link button"

        it "contains the path to the printout action" do
          expect( subject ).to include( meeting_reservations_export_csi_csv_path(id: meeting_with_reservation.id) )
        end
      end
    end
    #-- -----------------------------------------------------------------------
