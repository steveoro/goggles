...
describe Timing, :type => :model do
  ...
  shared_examples_for "(a new Timing with all members assigned)" do
    it "returns a Timing object" do
      expect( subject ).to be_an_instance_of( Timing )
    end
  ...
  context "[implemented methods]" do
    it_behaves_like( "(the existance of a method)",
      [
        :clear, :set_from_hundreds, :+, :-, :==, :<=>,
        :to_hundreds, :to_s
      ]
    )
    it_behaves_like( "(the existance of a class method)",
      [
        :to_s, :to_compact_s, :to_hour_string, :to_minute_string,
        :to_formatted_pause, :to_formatted_start_and_rest
      ]
    )
  end
  #-- -------------------------------------------------------------------------
  ...
  describe "#initialize" do
    context "[for an empty instance]" do
      subject { Timing.new() }
      it_behaves_like( "(a new Timing with all members at 0)" )
    end
    ...
    context "[for a non-zero instance]" do
      ...
      it_behaves_like( "(a new Timing with all members assigned)" )
    end
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  describe "#set_from_hundreds" do
    let(:fixture_hundreds)  { (rand * 10000).to_i }
    subject { Timing.new().set_from_hundreds( fixture_hundreds ) }
  ...
  describe "#to_hundreds" do
    ...
    it "has an equal value of hundreds" do
      expect( subject.to_hundreds ).to eq( fix1_hours * 360000 + fix1_mins * 6000 + fix1_secs * 100 + fix1_hundreds )
    end
  end
  #-- -------------------------------------------------------------------------
  #++
  ...
  describe "#<=>" do
    ...
    let( :fixture_prev )  { Timing.new( fixture.hundreds - rand*fixture.hundreds, fixture.seconds - rand*fixture.seconds, fixture.minutes - rand*fixture.minutes, fixture.hours ) }
    let( :fixture_succ )  { Timing.new( fixture.hundreds + rand*fixture.hundreds, fixture.seconds + rand*fixture.seconds, fixture.minutes + rand*fixture.minutes, fixture.hours ) }
  ...
  shared_examples_for "(a String containing specific values)" do
  ...
  describe "#to_s" do
    ...
    it_behaves_like( "(a String containing specific values)" )
