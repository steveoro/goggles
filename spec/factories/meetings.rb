...
FactoryGirl.define do
  factory :swimmin_pool do
    ...
    pool_type_id              { ((rand * 100) % 2).to_i }  # ASSERT: 25 and 50 meters type should exists
    city_id                   { ((rand * 100) % 50).to_i } # ASSERT: at least 50 cities
  end
  ...
  factory :meeting do
    ...
    edition_type_id           { ((rand * 100) % 5).to_i } # ASSERT: at least 5 edition types
    timing_type_id            { ((rand * 100) % 3).to_i } # ASSERT: at least 3 timing types
    user
  end
