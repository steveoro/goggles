...
describe RelaySwimmersCombiner, type: :strategy do
  # TODO Rewrite the spec domain using a random MeetingEvent like this:
  # me = MeetingEvent.includes(:event_type, meeting_session: :meeting).joins(:event_type, meeting_session: :meeting).where("event_types.code = 'S4x50MI'").sample
  # s4x50sl_meeting = me.meeting_session.meeting
  # => create manually fixture reservations for each swimmer that has to be tested
  # => make sure the swimmers badge are available too
  ...
  describe "#retrieve_swimmers" do
    # it "returns a non empty array for Reggio Emilia 2019" do
    #   expect( subject.retrieve_swimmers().size ).to be > 0
    # end
    # it "returns an array cotaining numbers" do
    #   expect( subject.retrieve_swimmers() ).to all( be > 0 )
    # end
    # it "returns an array cotaining swimmers ids" do
    #   swimmers_ids = subject.retrieve_swimmers()
    ...
    #   swimmers_ids = subject.retrieve_swimmers(male.id)
    #   swimmers_ids.each do |swimmer_id|
    #     expect( Swimmer.find(swimmer_id).gender_type_id ).to eq( male.id )
    #   end
    # end
    # it "returns only females swimmers if female gender specified" do
    #   swimmers_ids = subject.retrieve_swimmers(female.id)
    #   swimmers_ids.each do |swimmer_id|
