...
describe SeasonCreator, type: :strategy do
  ...
  context "with requested parameters" do
    ...
    describe "#edition," do
      it "is a valid number" do
        expect( subject.edition ).to be_a_kind_of( Number )
      end
    ...
    describe "#next_header_year," do
      ...
      it "returns a string representing a greater value if simple year given" do
        ...
        expect( subject.next_header_year( subject.older_season.header_year ) ).to be_a_kind_of( String )
        expect( subject.next_header_year( simple_year ).to_i ).to be > simple_year 
      end
      it "returns a string representing a greater couple of year if couple of year given" do
        ...
        expect( years[0] ).to be < years[1]
        expect( years[0] ).to be equal( years + 1 )
        expect( years[1] ).to be equal( years + 2 )
      end
    end
    #-- -----------------------------------------------------------------------
    ...
    describe "#renew_categories," do
      ...
      it "returns the same number and types of categories of the older season" do
        ...
        expect( subject.renew_categories.only_relays.count ).to be_equal( older_season.category_types.only_relays.count )
        expect( subject.renew_categories.are_not_relays.count ).to be_equal( older_season.category_types.are_not_relays.count )
      end
      it "returns the same category types code of older season" do
        new_categories = subject.renew_categories
        new_categories.each do |category_type|
          expect( older_season.category_types.find_by_code( category_type.code ).count).to be_equal( 1 )
        end
      end
    end
    describe "#categories," do
      it "is a collection of categories" do
        expect( subject.categories ).to be_a_kind_of( ActiveRecord::Relation )
    ...
    describe "#meetings," do
      it "is a collection of meetings" do
        expect( subject.meetings ).to be_a_kind_of( ActiveRecord::Relation )
