...
describe Api::V1::UserTrainingsController, type: :controller, api: true do
  before(:all) do # Force the creation of the required rows:
    ...
    @fixture_row = FactoryBot.create( :user_training, user_id: @user.id )
  end
  ...
  describe 'GET #index' do
    context "with :user_id filtering parameter" do
      before(:each) do
        get :index, format: :json, params: { user_id: @user.id, user_email: @user.email, user_token: @user.authentication_token }
      end
  ...
  describe 'POST #create/:user_training' do
    let(:post_attributes) { FactoryBot.attributes_for(:user_training, user_id: @user.id) }

    context "with a non-JSON request," do
      it "refuses the request" do
        post :create, params: { user_training: post_attributes, user_email: @user.email, user_token: @user.authentication_token }
        expect( response ).not_to be_a_success
    ...
    context "with a valid request but for an unlogged user," do
      it "refuses the request with unauthorized status" do
        post :create, format: :json, params: { user_training: post_attributes }
        expect( response ).not_to be_a_success
        expect(response.status).to eq( 401 ) # 401 = unauthorized
      end
      it "doesn't add a new row" do
    ...
    context "with non-valid attributes," do
      let(:invalid_post_attributes) { FactoryBot.attributes_for(:invalid_user_training) }
...
#        puts "\r\n- response.body: <<#{ response.body }>>"
  ...
  describe 'PUT #update/:id' do
    before(:all) do # Force the creation of the required rows:
      @updatable_row = FactoryBot.create( :user_training, user_id: @user.id )
      @put_attributes = FactoryBot.attributes_for(:user_training, user_id: @user.id, description: "Another description (UPDATED)")
      @invalid_put_attributes = FactoryBot.attributes_for(:invalid_user_training)
    end

    context "with a non-JSON request," do
      before(:each) do
        put :update, params: { id: @updatable_row.id, user_training: @put_attributes, user_email: @user.email, user_token: @user.authentication_token }
      end
      it "refuses the request" do
        expect( response ).not_to be_a_success
        expect(response.status).to eq( 406 ) # 406 = not acceptable
    ...
    context "with a valid request but for an unlogged user," do
      before(:each) do
        put :update, format: :json, params: { id: @updatable_row.id, user_training: @put_attributes }
      end
      it "refuses the request with unauthorized status" do
        expect( response ).not_to be_a_success
        expect(response.status).to eq( 401 ) # 401 = unauthorized
    ...
    context "with non-valid attributes," do
      before(:each) do
        put :update, format: :json, params: { id: @updatable_row.id, user_training: @invalid_put_attributes, user_email: @user.email, user_token: @user.authentication_token }
      end
      it "refuses the request" do
        expect( response ).not_to be_a_success
    ...
    context "with a valid request and credentials" do
      before(:each) do
        put :update, format: :json, params: { id: @updatable_row.id, user_training: @put_attributes, user_email: @user.email, user_token: @user.authentication_token }
      end
      it "handles successfully the request" do
        expect( response ).to be_a_success # 200 = success
      end
  ...
  describe '[DELETE destroy/:id]' do
    before :each do
      @deletable_row = FactoryBot.create( :user_training, user_id: @user.id )
    end
