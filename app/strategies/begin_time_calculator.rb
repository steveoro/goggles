...
#      puts "\r\nprevious_begin_time: #{previous_begin_time}"
  ...
  def self.compute_for_all( meeting, diff_sql_text, dry_run = false )
    if meeting.instance_of?( Meeting ) && diff_sql_text.instance_of?( String )
      ...
      program_results = []
      if results_count == 0
        diff_sql_text << "-- Using ENTRIES to compute heat length.\r\n\r\n"
      else
        diff_sql_text << "-- Using RESULTS to compute heat length.\r\n\r\n"
      end
      ...
      meeting.meeting_programs.order(:event_order).map do |mprg|
        ...
        if mprg.meeting_session.begin_time.instance_of?(Time) && mprg.meeting_session.begin_time.hour > 0
          session_begin_time = mprg.meeting_session.begin_time
        else                                        # Default start time: 08:30
          session_begin_time = mprg.meeting_session.scheduled_date.to_time + (8 * 60 * 60) + (30 * 60)
        end
        ...
        if program_results.last.instance_of?(Hash)
          program_begin_time = program_results.last[:program_begin] + program_results.last[:program_tot_secs]
        else
          program_begin_time = session_begin_time + 120
        end
