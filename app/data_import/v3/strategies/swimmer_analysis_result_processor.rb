...
=end
class V3::SwimmerAnalysisResultProcessor
  ...
  def run( swimmer_analysis_result, is_confirmed, swimmer_alias_override_id )
    ...
    if (! is_confirmed) || swimmer_analysis_result.can_insert_swimmer
      begin
        ...
        update_logs( "\r\n*** V3::SwimmerAnalysisResultProcessor: exception caught during DataImportSwimmer building! (Name:'#{swimmer_name}')", :error )
        update_logs( "*** #{ $!.to_s }\r\n", :error ) if $!
      ...
      begin
        DataImportSwimmerAlias.transaction do          # Let's make sure other threads have not already done what we want to do:
          if ( DataImportSwimmerAlias.where(complete_name: swimmer_name, swimmer_id: swimmer_id).none? )
            ...
            @logger.info( "\r\n*** V3::SwimmerAnalysisResultProcessor: WARNING: skipping DataImportSwimmerAlias creation because was (unexpectedly) found already existing! (Name:'#{swimmer_name}', swimmer_id:#{swimmer_id})" ) if @logger
          end
        end
      rescue
        update_logs( "\r\n*** V3::SwimmerAnalysisResultProcessor: exception caught during DataImportSwimmerAlias save! (Name:'#{swimmer_name}', swimmer_id:#{swimmer_id})", :error )
        update_logs( "*** #{ $!.to_s }\r\n", :error ) if $!
        @flash[:error] = "#{I18n.t(:something_went_wrong)} ['#{ $!.to_s }']"
        is_ok = false
      end
