...
=end
class V3::TeamAnalysisResultProcessor
  ...
  def run( team_analysis_result, is_confirmed, team_alias_override_id )
    ...
    if (! is_confirmed) || team_analysis_result.can_insert_team
      begin
        ...
        update_logs( "\r\n*** V3::TeamAnalysisResultProcessor: exception caught during DataImportTeam building! (Name:'#{team_name}')", :error )
        update_logs( "*** #{ $!.to_s }\r\n", :error ) if $!
      ...
      begin
        DataImportTeamAlias.transaction do          # Let's make sure other threads have not already done what we want to do:
          if ( DataImportTeamAlias.where(name: team_name, team_id: team_id).none? )
            ...
            @logger.info( "\r\n*** V3::TeamAnalysisResultProcessor: WARNING: skipping DataImportTeamAlias creation because was (unexpectedly) found already existing! (Name:'#{team_name}', team_id:#{team_id})" ) if @logger
          end
        end
      rescue
        update_logs( "\r\n*** V3::TeamAnalysisResultProcessor: exception caught during DataImportTeamAlias save! (Name:'#{team_name}', team_id:#{team_id})", :error )
        update_logs( "*** #{ $!.to_s }\r\n", :error ) if $!
        @flash[:error] = "#{I18n.t(:something_went_wrong)} ['#{ $!.to_s }']"
        is_ok = false
      end
      ...
      begin
        TeamAffiliation.transaction do              # Let's make sure other threads have not already done what we want to do:
          if ( TeamAffiliation.where(
            ...
            update_logs( "\r\n*** V3::TeamAnalysisResultProcessor: WARNING: skipping TeamAffiliation creation because was (unexpectedly) found already existing! (Name:'#{team_name}', team_id:#{team_id}, season_id:#{season_id})", :error )
          end
        end
      rescue
        update_logs( "\r\n*** V3::TeamAnalysisResultProcessor: exception caught during TeamAffiliation save! (Name:'#{team_name}', team_id:#{team_id}, season_id:#{season_id})", :error )
        update_logs( "*** #{ $!.to_s }\r\n", :error ) if $!
        @flash[:error] = "#{I18n.t(:something_went_wrong)} ['#{ $!.to_s }']"
        is_ok = false
      end
