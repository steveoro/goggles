...
class MiscController < ApplicationController
  ...
  def fin_score_calculation
    ...
    if request.post?                                # === POST: ===
      category_type_id = params[:category_type] && params[:category_type][:id] ? params[:category_type][:id].to_i : 0
      gender_type_id   = params[:gender_type] && params[:gender_type][:id] ? params[:gender_type][:id].to_i : 0
